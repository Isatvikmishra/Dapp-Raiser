...
export const CampaignCard: React.FC<CampaignCardProps> = ({ campaignAddress }) => {
    ...
    const truncatedDescription = campaignDescription?.length > 100 ? `${campaignDescription.slice(0, 150)}...` : campaignDescription;
    ...
    return (
            <div className="flex flex-col justify-between max-w-sm p-6 bg-white border border-slate-200 rounded-lg shadow">
                ...
                <Link
                    ...
                    passHref={true}
                >
                    <p className="inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white bg-teal-500 rounded-lg hover:bg-teal-600 focus:ring-4 focus:outline-none focus:ring-teal-300 dark:bg-teal-600 dark:hover:bg-teal-700 dark:focus:ring-teal-800">
                        View Campaign
